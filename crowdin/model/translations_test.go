...
func TestBatchPreTranslationRequestValidate(t *testing.T) {
	tests := []struct {
		name  string
		req   *BatchPreTranslationRequest
		err   string
		valid bool
	}{
		{
		...
		{
			name: "missing path",
			req:  &BatchPreTranslationRequest{Op: "replace"},
			err:  "path is required",
		},
		{
			name: "missing value",
			req:  &BatchPreTranslationRequest{Op: "replace", Path: "/some/path"},
